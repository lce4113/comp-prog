// ––– CodeForces Round #710: Problem E –––
...
void solve(int32_t T) {
  ...
  st b, d;
  FOR(i, 0, n) {
    if (b.count(a[i])) d.ins(i);
    b.ins(a[i]);
  }

  st c;
  FOR(i, 1, n + 1) if (!b.count(i)) c.ins(i);

  vi a1 = copy(a), a2 = copy(a);
  st y = c;
  each(d, x) a1[x] = min(c), c.erase(min(c));
  each(d, x) a2[x] = *(--y.lb(a[x])), y.erase(a2[x]);
