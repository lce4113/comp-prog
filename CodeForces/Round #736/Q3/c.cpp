// ––– CodeForces Round #736: Problem C –––
...
void solve(int32_t T) {
  ...
  V<S<int>> b(n);
  FOR(i, 0, n) b[i].insert(i);
  FOR(i, 0, m) {
    a[i].f--, a[i].s--;
    b[a[i].f].insert(a[i].s);
    b[a[i].s].insert(a[i].f);
  }
  S<int> ans;
  FOR(i, 0, n) ans.ins(i);
  FOR(i, 0, n) if (*b[i].rbegin() != i) ans.erase(i);
  ...
  while (q--) {
    ...
    if (x == 1) {
      ...
      b[u].erase(v);
      b[v].erase(u);
      if (*b[min(u, v)].rbegin() == min(u, v)) ans.insert(min(u, v));
    } else {
      out(sz(ans));
    }
    /* out(ans); */
