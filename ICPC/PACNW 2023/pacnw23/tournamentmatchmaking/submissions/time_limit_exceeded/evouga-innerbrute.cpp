...
bool compatible(group &g1, group &g2)
{
    ...
    if(s1 + s2 != teamsize)
        return false;
    uint64_t max1 = (1ULL << s1);
    uint64_t max2 = (1ULL << s2);
    for(uint64_t i=0; i<max1; i++)
    {
        for(uint64_t j=0; j<max2; j++)
        {
            std::set<int> roles;
            for(int k=0; k<s1; k++)
            {
                if(i & (1ULL << k))
                {
                    roles.insert(g1[k].first);
                }
                else
                {
                    roles.insert(g1[k].second);
                }
            }
            for(int k=0; k<s2; k++)
            {
                if(j & (1ULL << k))
                {
                    roles.insert(g2[k].first);
                }
                else
                {
                    roles.insert(g2[k].second);
                }
            }
            if(roles.size() == teamsize)
                return true;
        }
    }
    return false;
