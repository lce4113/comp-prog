...
#define midpoint(x,y) ((x+y)>>1)

using pt=complex<ll>;
ll xx(pt x,pt y) { return imag(x*conj(y)); }
bool LEF(pt x,pt y,pt z) { return xx(y-x,z-y)<0; }
bool RIG(pt x,pt y,pt z) { return xx(y-x,z-y)>0; }
...
void f(int l,int r) {
	...
	for(int i=l;i<=r;i++) pts.pb({z[i],i});
	sort(all(pts),[&](auto x,auto y) { return imag(x.fi)<imag(y.fi); });
	vector<pt> lefs={{-inf,0}},rigs={{inf,0}};
	for(auto [p,i]:pts) {
		auto &qh=i<=ml?rigs:lefs;
		int L=0,R=sz(qh)-1;
		for(;L<R;) {
			int M=midpoint(L,R);
			if((i<=ml?LEF:RIG)(qh[M],p,qh[M+1])) L=M+1;
			else R=M;
		}
		auto &relb=i<=ml?rig[i]:lef[i];
		if((i<=ml?LEF:RIG)(relb,p,qh[L])) relb=qh[L];

		auto &ah=i<=ml?lefs:rigs;
		if(i<=ml && imag(p)==imag(ah.back()) && real(p)<=real(ah.back())) continue;
		if(i>=mr && imag(p)==imag(ah.back()) && real(p)>=real(ah.back())) continue;
		for(;sz(ah)>=2;) {
			if(i<=ml && imag(p)==imag(ah.back()) && real(p)>real(ah.back())) { ah.pop_back(); continue; }
			if(i>=mr && imag(p)==imag(ah.back()) && real(p)<real(ah.back())) { ah.pop_back(); continue; }
			if((i<=ml?RIG:LEF)(p,ah.back(),ah[sz(ah)-2])) { ah.pop_back(); continue; }
			break;
		}
		ah.pb(p);
	}
...
void _main() { /* CURSOR START */
	int n; ine(n,arg1); assert(inb(n,1,e5));
	for(int i=1;i<=n;i++) {
		int x,y,l,r; inr(x,-e6,e6), inr(y,e6), inr(l,-e6+2*e6/n*i), inr(r,l,l+e6/n);
		assert(inb(x,-e6,e6));
		assert(inb(y,1,e6));
		assert(-e6<=l && l<=r && r<=e6);
		if(i>1) assert(l>real(lef[i-1]));
		// static set<pair<int,int>> ss;
		// assert(ss.insert({x,y}).se);
		z[i]={x,y};
		lef[i]={l,0};
		rig[i]={r,0};
	}
	...
	flt Z=0;
	for(int i=1;i<=n;i++) {
		// dbg(i,z[i],lef[i],rig[i]);
		if(LEF(lef[i],z[i],rig[i])) return cout<<-1<<endl,nop;
		pt dd;
		if(inb(real(z[i]),real(lef[i]),real(rig[i]))) dd={real(z[i]),0};
		else if(real(z[i])<real(lef[i])) dd=lef[i];
		else if(real(z[i])>real(rig[i])) dd=rig[i];
		else assert(0);
		dd-=z[i];
		complex<flt> DD={(flt)real(dd),(flt)imag(dd)};
		Z+=abs(DD)*imag(z[i])/imag(-dd);
	}
	cout<<fixed<<setprecision(22)<<Z<<endl;
}
