...
int main(int argc, char** argv) {
  int r = in.get_int(1, 20); in.get_space();
  int c = in.get_int(1, 20); in.get_eol();
  for(int q = 0; q < 2; q++) {
    set<int> s;
    for(int i = 0; i < r; i++) {
      vector<int> v(c);
      for(int j = 0; j < c; j++) {
        v[j] = in.get_int(1, r*c);
        if(j == c-1) in.get_eol();
        else in.get_space();
      }
      for(int j = 1; j < c; j++) fassert(v[j] > v[j-1], "not increasing");
      for(int out: v) s.insert(out);
    }
    fassert(sz(s) == r*c, "not a permutation");
  }
	in.get_eof();
}