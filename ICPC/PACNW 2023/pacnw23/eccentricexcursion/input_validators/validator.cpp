...
int main(int argc, char** argv) {
  int n = in.get_int(1, 500); in.get_space();
  int k = in.get_int(0, n-1); in.get_eol();
  disjoint_set dsu(n+1);
  for(int i = 0; i < n-1; i++) {
    int a = in.get_int(1, n); in.get_space();
    int b = in.get_int(a+1, n); in.get_eol();
    fassert(dsu.merge(a, b), "not a tree");
  }
	in.get_eof();
}