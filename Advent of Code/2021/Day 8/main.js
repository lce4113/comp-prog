...
a = `begfcd fabg aecgbdf cefagb edgcba eacbf efgbc bca ab decfa | cbgef befdcg bceaf fagb
...
function test(map, strings) {
  things = []
  for (const x of strings) {
    z = getNum(x.split('').map(y => map[y]).join(''))
    if (z == -1) return false
    if (things.includes(z)) return false
    things.push(z)
  }
  // console.log(things);
  return true
...
for (const c of b) {
  for (const p of ps) {
    ...
    if (test(map, c[0].split(' '))) {
      num = ""
      for (const x of c[1].split(' ')) {
        num += getNum(x.split('').map(y => map[y]).join(''))
        // console.log(x.split('').map(y => map[y]).join(''));
      }
      // console.log(map);
      // console.log(num);
      ans += Number(num)
