// ––– CodeForces AtCoder Beginner 232: Problem C –––
...
tcT struct is_readable<T, typename std::enable_if_t<is_same_v<decltype(cin >> declval<T&>()), istream&>>>
    : true_type {
};
...
tcT struct is_printable<T, typename std::enable_if_t<is_same_v<decltype(cout << declval<T>()), ostream&>>>
    : true_type {
};
...
void solve(int32_t T)
{
  ...
  st sa, sb;
  each(ma, x) sa.insert(x.s);
  each(mb, x) sb.insert(x.s);
