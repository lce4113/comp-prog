// ––– CodeForces Code Jam Round 1B 2022: Problem B –––
...
void solve(int32_t T) {
  ...
  FOR(i, 1, n) {
    c.pb({
        {abs(b[i - 1].f - b[i].s), abs(b[i - 1].f - b[i].f)},
        {abs(b[i - 1].s - b[i].s), abs(b[i - 1].s - b[i].f)},
    });
  }

  ll l = min(c.bk.f.f, c.bk.f.s), r = min(c.bk.s.f, c.bk.s.s);
  /* out(l, r); */
  ROF(i, n - 2, 0) {
    ll x = min(c[i].f.f + l, c[i].f.s + r);
    ll y = min(c[i].s.f + l, c[i].s.s + r);
    l = x, r = y;
    /* out(l, r); */
  }
  l += b[0].s, r += b[0].f;
