// ––– CodeForces Round #762: Problem C –––
...
str ad(int d, int c) {
  ...
  str ans = "";
  for (int i = 0, k = 0; i < sz(da) || k < sz(dc); i++, k++) {
    ans = to_string((k < sz(da) ? da[i] : 0) + (k < sz(dc) ? dc[k] : 0)) + ans;
  }
  ...
  for (int a = 1; a <= 10000; a++) {
    for (int b = 1; b <= 10000; b++) {
      ...
      while (k < sz(db)) {
        out(i, k);
        if (da[i] == 0) i++;
        if (db[k] < (i < sz(da) ? da[i] : 0)) {
          if (k + 1 < sz(db) && db[k + 1] == 1) {
            ans += (10 + db[k++] - da[i]) * pow(10, l++);
          } else {
            B = true;
            break;
          }
        } else {
          ans += (db[k] - (i < sz(da) ? da[i] : 0)) * pow(10, l++);
        }
        i++, k++;
      }
      if (i < sz(da) || B) {
      ...
      /* out(ans); */
      if (ad(a, ans) != to_string(b)) {
        out(a, b, ad(a, ans), ans);
      }
    }
  }
}
