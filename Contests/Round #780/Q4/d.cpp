// ––– CodeForces Round #780: Problem D –––
...
void solve(int32_t T) {
  ...
  vi zeroes(1, -1);
  FOR(i, 0, n) if (a[i] == 0) zeroes.pb(i);
  zeroes.pb(n);

  int maxtwos = -1, l, r;
  FOR(i, 0, sz(zeroes) - 1) {
    int x = zeroes[i] + 1, y = zeroes[i + 1];
    vi negs;
    int twos = 0;
    FOR(k, x, y) {
      if (a[k] < 0) negs.pb(k);
    ...
    /* out(negs, twos); */
    if (sz(negs) % 2) {
      int ltwos = 0, Ltwos = 0, rtwos = 0, Rtwos = 0;
      FOR(k, x, negs[0]) if (abs(a[k]) == 2) ltwos++;
      if (ltwos > maxtwos) maxtwos = ltwos, l = x, r = negs[0];
      FOR(k, negs[0] + 1, y) if (abs(a[k]) == 2) Ltwos++;
      if (Ltwos > maxtwos) maxtwos = Ltwos, l = negs[0] + 1, r = y;
      FOR(k, x, negs.bk) if (abs(a[k]) == 2) rtwos++;
      if (rtwos > maxtwos) maxtwos = rtwos, l = x, r = negs.bk;
      FOR(k, negs.bk + 1, y) if (abs(a[k]) == 2) Rtwos++;
      if (Rtwos > maxtwos) maxtwos = Rtwos, l = negs.bk + 1, r = y;
    } else {
      if (twos > maxtwos) maxtwos = twos, l = x, r = y;
    }
  }
