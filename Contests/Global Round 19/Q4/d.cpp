// ––– CodeForces Global Round 19: Problem D –––
...
void solve(int32_t T) {
  ...
  FOR(i, 0, n) {
    if ((suma - a[i]) * b[i] + (sumb - b[i]) * a[i] <
        (suma - a[i]) * a[i] + (sumb - b[i]) * b[i]) {
      /* ll tmp = a[c[i][2]]; */
      /* a[c[i][2]] = b[c[i][2]]; */
      /* b[c[i][2]] = tmp; */
      suma -= a[i], suma += b[i];
