// ––– CodeForces Round #785: Problem C –––
...
int to_pal(int x, int asdf) {
  vi y = dg(x);
  x *= exp(10, sz(y) - asdf);
  FOR(i, asdf, sz(y)) x += y[i] * exp(10, sz(y) - i - 1);
  rtn x;
}
...
mp rec(int n) {
  ...
  ll curr = 0;
  mp m;
  for (int k = 0; pals[k] <= n; k++) {
    curr += rec(n - pals[k])[min(n - pals[k], pals[k])];
    curr %= MOD;
    m[pals[k]] = curr;
  }
  m[n] = m.rbegin()->s;
...
void solve(int32_t T) {
  FOR(i, 1, 100) pals.pb(to_pal(i, 0));
  FOR(i, 1, 401) pals.pb(to_pal(i, 1));
  sor(pals);
  dp[0][0] = 1;
