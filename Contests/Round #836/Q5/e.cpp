// ––– CodeForces Round #836: Problem E –––
...
bool comp(vi x, vi y) {
  FOR(i, 0, sz(x)) {
    if (x[i] != -1 && y[i] == -1) rtn true;
    if (x[i] == -1 && y[i] != -1) rtn false;
  }
  rtn false;
}
...
void solve(int32_t T) {
  ...
  vv b(m, vi(n));
  FOR(i, 0, n) FOR(k, 0, m) b[k][i] = a[i][k];
  sort(b.begin(), b.end(), comp);

  FOR(l, 0, 2) {
    FOR(i, 0, m - 1) {
      vi x;
      FOR(k, 0, n) if (b[i][k] != -1 && b[i + 1][k] != -1) x.pb(k);
      if (sz(x) == 0) continue;
      int y = (b[i + 1][x[0]] - b[i][x[0]] + h) % h;
      FOR(k, 1, sz(x)) {
        if (y != (b[i + 1][x[k]] - b[i][x[k]] + h) % h) {
          out(0);
          rtn;
        }
      }
  ...
  FOR(i, 0, m) FOR(k, 0, n) if (b[i][k] != -1) b[i][k] = 1;

  FOR(l, 0, 2) {
    FOR(i, 0, m - 1) {
      bool x = false;
      FOR(k, 0, n) if (b[i][k] == 1 && b[i + 1][k] == 1) x = true;
      if (x) {
        FOR(k, 0, n) {
          if (b[i][k] != -1) b[i + 1][k] = 1;
          if (b[i + 1][k] != -1) b[i][k] = 1;
        }
      }
    }
  }
  ...
  FOR(i, 0, m - 1) {
    FOR(k, 0, n - 1) {
      if (b[i][k] == 1 && b[i + 1][k + 1] == 1 && b[i][k + 1] == -1 &&
          b[i + 1][k] == -1) {
        y++;
      }
      if (b[i][k] == 1 && b[i + 1][k + 1] == -1 && b[i][k + 1] == -1 &&
  ...
  FOR(i, 0, m - 1) {
    if (b[i][n - 1] == 1 && b[i + 1][n - 1] == -1) {
      y += m - i - 1;
    }
  }
  FOR(k, 0, n - 1) {
    if (b[m - 1][k] == 1 && b[m - 1][k + 1] == -1) {
      y += n - k - 1;
    }
  }
  if (b[0][0] == -1) {
