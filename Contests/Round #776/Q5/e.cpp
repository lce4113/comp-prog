// ––– CodeForces Round #776: Problem E –––
...
tcTUU T max(T a, U... b) { rtn max(a, max(b...)); }
...
void solve(int32_t T) {
  ...
  V<pi> b;
  FOR(i, 1, n) b.pb({a[i] - a[i - 1] - 1, i});
  b.pb({a[0] - 1, 0}), b.pb({a.bk - 1, n});
  sor(b);

  int nw1 = b[0].s == 0 ? a[1] - 1 : a[0] - 1;
  nw1 = max(nw1, b[0].s == n ? d - a[n - 2] - 1 : d - a.bk - 1);
  nw1 = max(nw1, (b.bk.f - 1) / 2);
  int nw2;
  if (b[0].s == n - 1) {
    nw2 = d - a[b[0].s - 1] - 1;
  } else {
    nw2 = a[b[0].s + 1] - a[b[0].s - 1] - 1;
  }
  out(nw1, nw2);
  int ans1 = min(nw1, nw2);

  /*   nw1 = b[0].s == 0 ? a[1] - 1 : a[0] - 1; */
  /*   nw1 = max(nw1, b[0].s == n ? d - a[n - 2] - 1 : d - a.bk - 1); */
  /*   nw1 = max(nw1, b.bk.f / 2 - 1); */
  /*   if (b[0].s == 1) { */
  /*     nw2 = a[b[0].s - 1] - 1; */
  /*   } else { */
  /*     nw2 = a[b[0].s] - a[b[0].s - 2]; */
  /*   } */
  /*   out(nw1, nw2); */
  /*   int ans2 = min(nw1, nw2); */
