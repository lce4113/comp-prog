// ––– CodeForces Educational Round 122: Problem D –––
...
M<pi, int> dp;
int rec(vi d, vi c, int n, int pos, int k) {
  if (dp.count({pos, k})) return dp[{pos, k}];
  if (pos == n - 1) return (k >= d[n - 1] ? c[n - 1] : 0);
  int mx = 0;
  FOR(i, pos + 1, n) {
    if (k >= d[i]) {
      int next = c[pos] + rec(d, c, n, i, k - d[i]);
      mx = max(mx, next);
    }
  }
  out(pos, k, mx);
