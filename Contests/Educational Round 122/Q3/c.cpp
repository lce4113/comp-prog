// ––– CodeForces Educational Round 122: Problem C –––
...
void solve(int32_t T) {
  ll hc, dc, hm, dm, k, ds, hs;
  in(hc, dc, hm, dm, k, ds, hs);

  FOR(i, 0, k + 1) {
    ll dk = i, hk = k - i;
    ll ndc = dc + dk * ds, nhc = hc + hk * hs;
    /* out(nhc, dm, hm, ndc); */
    ll cturn = ceil((db)nhc / dm), mturn = ceil((db)hm / ndc);
    /* out(cturn, mturn); */
    if (cturn >= mturn) goto a;

    /* ll goal = mturn + i; */
    /* ll hgoal = dm * (goal - 1) + 1, dgoal = floor((db)hm / goal); */
    /* ll coins = ceil((db)(hgoal - hc) / hs) + ceil((db)(dgoal - dc) / ds); */
    /* out(coins); */
    /* if (coins <= k) goto a; */
  }
