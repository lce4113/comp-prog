// ––– CodeForces Round #782: Problem D –––
  ...
  FOR(i, 0, n) {
    ...
    if (ans[i] == 0) {
      if (a[i] != z2) {
        ans[i + a[i]] = 0;
        if (x == 1) {
          FOR(k, i + 1, i + a[i]) ans[k] = 1;
        } else if (a[i] == x + 1) {
          FOR(k, i + 1, i + a[i]) ans[k] = 0;
        }
      }
    } else {
      if (a[i] != o2) {
        ans[a[i]] = 0;
        if (x == 0) {
          FOR(k, i + 1, a[i]) ans[k] = 1;
        } else if (a[i] - i == x + 1) {
          FOR(k, i + 1, a[i]) ans[k] = 0;
        }
      }
    }
