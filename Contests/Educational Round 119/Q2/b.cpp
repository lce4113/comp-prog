// ––– CodeForces Educational Round 119: Problem B –––
...
void solve(int32_t T) {
  ...
  FOR(i, 0, 4) {
    ...
    if (i == 0) mx = max(a) - min(a);
    if (i == 1) mx = max(mx, max(a) - min(a));
    if (i == 2) my = max(a) - min(a);
    if (i == 3) my = max(my, max(a) - min(a));
  }

  ll ans = max(mx * h, my * w);
  out(ans);
}
