// ––– CodeForces Educational Round 132: Problem B –––
...
void solve(int32_t T) {
  ...
  vl pfs1(n, 0), pfs2(n, 0);
  FOR(i, 0, n - 1) {
    pfs1[i + 1] = pfs1[i] + d1[i];
    pfs2[i + 1] = pfs2[i] + d2[i];
  }

  FOR(i, 0, m) {
    if (b[i].s > b[i].f) {
      out(pfs2[b[i].s - 1] - pfs2[b[i].f - 1]);
    } else {
      out(pfs1[b[i].f - 1] - pfs1[b[i].s - 1]);
    }
  }
}
