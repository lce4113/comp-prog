// ––– CodeForces Educational Round 124: Problem C –––
...
void solve(int32_t T) {
  ...
  vl mn(4, 1e9);
  FOR(i, 0, n) {
    mn[0] = min(mn[0], abs(a[i] - b[0]));
    mn[1] = min(mn[1], abs(a[i] - b.bk));
    mn[2] = min(mn[2], abs(b[i] - a[0]));
    mn[3] = min(mn[3], abs(b[i] - a.bk));
  }
  ll ans1 = sum(mn);
  ll ans21 = abs(b[0] - a[0]) + abs(b.bk - a.bk);
  ll ans22 = mn[0] + mn[2] + abs(b.bk - a.bk);
  ll ans23 = abs(b[0] - a[0]) + mn[1] + mn[3];
  ll ans31 = abs(b[0] - a.bk) + abs(b.bk - a[0]);
  ll ans32 = mn[0] + mn[3] + abs(b.bk - a[0]);
  ll ans33 = abs(b[0] - a.bk) + mn[1] + mn[2];
  out(min({ans1, ans21, ans22, ans23, ans31, ans32, ans33}));
}
